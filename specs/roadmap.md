...
- Ames-coordinated, Fine-backed repository snapshots with incremental have negotiation, a bounded initial-response deadline, byte-bounded 4 MiB object pages, an eight-read sliding window, Ames `%prog`/`%rate` fragment telemetry, exact-path `%yawn` cancellation, fragment-aware idle detection, Dojo transfer diagnostics, status pruning, object-count checks, and per-object OID verification
