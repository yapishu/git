...
- bounded remote repository overviews over Ames, with acknowledged deferred preparation, metadata-only tree flattening, bounded history counts, deduplicated commit summaries, one-noun Mesa delivery, progress-aware Fine compatibility, structurally bounded per-file commit attribution, revision-stamp, commit-detail, and individual-file reads accepted; browser-local indefinite overview caching with explicit refresh and update detection; and separate unanswered-request, preparation, and stalled-read termination
