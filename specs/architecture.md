...
Public repositories can be forked directly from another ship. The receiver sends the OIDs it already has over an Ames coordination poke. If the source does not answer that initial request within 45 seconds, the receiver terminates the preflight instead of retaining an unstarted transfer that later requests would deduplicate onto. The source groups missing immutable objects into transfer pages bounded at 4 MiB of object data and 1,024 objects, publishes each page as one revision of a transfer-scoped Fine spur, then announces its symbolic head, refs, object count, and page count. An object larger than the byte ceiling occupies a page by itself. This gives Fine a coarse noun to fragment, acknowledge, retransmit, and congestion-control, avoiding a fresh Fine request and congestion window for every small group of Git objects. The receiver keeps at most eight `%keen` reads in flight and advances that window only as Fine responses arrive; Vere handles network fragmentation and reassembly instead of Gall sending an application-level chunk sequence. Cancellation issues `%yawn` with the exact `[ship scry-path]` identity used by every outstanding `%keen`, then sends a source release so the request-scoped Fine spur is culled. A zero-object transfer still publishes one empty page so the same handshake completes when both sides already possess the graph. Gall recomputes every Git OID, rejects duplicate or excess pages, checks the announced count and complete ref graph, installs the repository atomically, and sends a release poke so the source can cull the transient revisions. A receiver declares failure only after two minutes without verified Fine progress, so a live transfer has no fixed completion deadline; unreleased source snapshots expire after ten minutes. Timer error notifications are consumed without retry. The authenticated API exposes announced and verified object counts for progress, coalesces identical active forks onto one transfer, retains a cancelled terminal result long enough for the initiating browser to observe it, and can prune consumed results. A self-fork takes the same validation path but bypasses the network read.
