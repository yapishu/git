...
Peer identities themselves are persisted as bookmarks and expanded on demand in the sidebar. Repository browsing uses a short Ames handshake to publish a point-in-time JSON overview at a request-scoped Fine spur under the `%urgit` Gall namespace (`/g/x/1/urgit//1/...`). The requesting ship reads revision 1 of that immutable spur and immediately asks the source to cull it. The overview contains public repository metadata, branch refs, the current file tree, and bounded first-parent commit history; it contains no object bodies or credentials. Cancellation and the 30-second requester timeout both ask the source to cull the request-scoped value.
