...
Peer identities themselves are persisted as bookmarks and expanded on demand in the sidebar. Repository browsing requests a bounded JSON overview through the same Ames coordination channel as catalog discovery. A source may instead announce a request-scoped Fine overview, which the requester reads and releases through the same identity-checked operation. The source responds only for a public repository. The overview contains repository metadata, branch refs, the current file tree, and bounded first-parent commit history; it contains no object bodies or credentials. The requester verifies that the responding ship, requested repository, and embedded repository identity all match its pending request before exposing the result. Concurrent identical requests are coalesced by the receiving agent, and an unanswered request becomes an explicit failure after 45 seconds rather than remaining active indefinitely. The frontend renders the peer-reported stage while active and replaces the loader with the terminal error and an explicit retry action.

Public repositories can be forked directly from another ship. The receiver sends the OIDs it already has over an Ames coordination poke. If the source does not answer that initial request within 45 seconds, the receiver terminates the preflight instead of retaining an unstarted transfer that later requests would deduplicate onto. A new request from the same ship for the same repository supersedes and culls any unreleased source snapshot from an earlier attempt, so retries do not accumulate duplicate Fine state. The source groups missing immutable objects into transfer pages bounded at 512 KiB of object data and 256 objects, publishes each page as one revision of a transfer-scoped Fine spur, then announces its symbolic head, refs, object count, and page count. An object larger than the byte ceiling occupies a page by itself. This gives Fine a moderate noun to fragment, acknowledge, retransmit, and congestion-control without creating a fresh transfer for every Git object. The receiver keeps exactly one `%keen` read in flight and requests the next revision only after validating the current page; Vere handles network fragmentation and reassembly instead of Gall sending an application-level chunk sequence. Each read also registers Ames `%prog` interest, and `%rate` gifts report Fine's received and total 1 KiB fragment counts. Cancellation issues `%yawn` with the exact `[ship scry-path]` identity used by the outstanding `%keen`, then sends a source release so the request-scoped Fine spur is culled. A zero-object transfer still publishes one empty page so the same handshake completes when both sides already possess the graph. Gall recomputes every Git OID, rejects duplicate or excess pages, checks the announced count and complete ref graph, installs the repository atomically, and sends a release poke so the source can cull the transient revisions. A receiver declares failure only after two minutes without either increasing Ames fragment counts or completing a verified page, so a live transfer has no fixed completion deadline; unreleased source snapshots expire after ten minutes. Timer error notifications are consumed without retry. The authenticated API exposes announced and verified object counts, page counts, and aggregate Fine fragment counts for progress, coalesces identical active forks onto one transfer, retains a cancelled terminal result long enough for the initiating browser to observe it, and can prune consumed results. `:urgit +dbug [%state 'transfers']` exposes the same transient transfer records in Dojo without placing them in persisted state. A self-fork takes the same validation path but bypasses the network read.
