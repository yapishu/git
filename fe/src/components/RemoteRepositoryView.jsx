...
export default function RemoteRepositoryView({ ship, data, onFork }) {
  ...
  return <main className="content">
    ...
    <section className="repo-body">
      {tab === 'code' && <div className="table"><div className="table-head"><span>Path</span><span>Size</span></div>{(data.files?.files || []).map((file) => <div className="table-row" key={file.path}><span className="file-path"><i />{file.path}</span><span className="quiet" title={exactBytes(file.size)}>{formatBytes(file.size)}</span></div>)}</div>}
