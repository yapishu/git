...
++  on-agent
  ...
  ?~  p.sign
    ...
    (receive-payload 'ok' (receive-results commands.pending %.y ''))
  ...
  %+  weld
    ...
    ==
  %+  give-simple-payload:app:server  eyre-id.pending
  (receive-payload 'ok' (receive-results commands.pending %.n message))
++  on-arvo
  |=  [=wire =sign-arvo]
  ^-  (quip card _this)
  =/  error-cards
  ...
  ::
    ...
    %-  receive-payload
    :*  'ok'
        (receive-results commands.pending %.n 'Clay update timed out without a result')
    ==
  ::
      [%clay-logs ~]
    ?.  ?=([%dill %logs *] sign-arvo)
